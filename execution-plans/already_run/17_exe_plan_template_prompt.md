...
- prompt: >
  ...
  - refactor the cap ingest, you must always have a workflow.py for orchestration 
  - also change the route so that we can use the datasources value to orientate to pick the correct workflow for each data_source, so it must be based on the dat source type param. also you are going to have a workflow for each type. So you might have a folder workflows/workflow_file? for this so that everything is handled properly and accordingly? and you will pass after correct verif the data to the handler which will call each workflow accordingly and return the response...So make the change: (is there a better and beyond level way? so just propose it as well)
