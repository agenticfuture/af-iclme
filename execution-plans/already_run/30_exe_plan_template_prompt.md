...
- prompt: >
  ...
  - so everything must be configured correctly cause now we have 404: this page could not be found, when loading the playground with the services
  - also you can now implement the all services action so that it fetches all the services and all their config from the db and present them, remove all the mock services
...
now go adhead and perform everything
  ...
  - link the name of the execution_plan or id: here just use the name of the file to the CHG-id (just in the spec,indicate the execution plan that triggered it)
