...
- prompt: >
  ...
  - still following that, update under the domain embeddings, the caps search_qdrant and embed_store_qdrant to adjust with the realm (teant_id,org_id,service_id) whenever they are saving embedding in qdrant so that everything could be well correlated and also make sure that when a request is sent from the chatbot ui with all those params and the query that they flow through the cap handle_request is working accordingly and well and that nothing is missing so that when a user has a request from the chatbot ui, it works perfectly and the embeddings are retrieved accordingly 
  - also ensure that when a new service is created and the embeddings for the file data sources are also executed accordingly and done properly and well...
  - so make sure everything wire perfectly and well...
