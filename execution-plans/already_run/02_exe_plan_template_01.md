...
- prompt: >
    from the current context you have from servicegensaas both frontend and backend, analyse the current backend and frontend and perform the following action correctly:
    - in service details: add like a drop down with a list of department/domain and also the possibility for the user to enter its own department/domain
    - for the bot configuration, add a suggestion button to fill the field "bot name", "description" and "system prompt" based on the service details provided earlier. 
    - in service creation: for the data source, also add the tag "coming soon" to url website and aws s3 bucket.
    - for channels, add the tag comming soon to all of them
    - for agent assignment, also add the tag coming soon.
    - each time we click on the "next button" we must send the data to the server immediately and collect necessary data from the server to continue the flow
    - change the label of the button at the end "create service" to "finish"
    - it's now that when we click on finish that we have the loading bar as you did and we must be redirected to the service dashboard itself and it's playground to be able to see the bot in action and the notification must appear in the notifications zone to inform the user of the state of the bot deployment.
