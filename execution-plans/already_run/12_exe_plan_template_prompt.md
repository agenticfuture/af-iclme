...
- prompt: >
  ...
  - after each step, when we click on next, it must send a post request with the information to be saved in the db based on the service_id, chatbot_id,
  and all the required id (tenant,org)
  ...
  - so even if for now we have steps (channels & agent assignment) that are coming soon, you can just simulate a post here for now to be completed in the future but the rest must work wonderfully and perfectly.
