...
- prompt: >
    from the current context you have from servicegensaas both frontend and backend, analyse the current backend and frontend and produce the correct mapping for the service creation so that when we click on the "create service" button, it launches the flow and create the service accordingly going through all the stages of the flow to have the service created. 
    also, for now we need to add the tag "coming soon" to some section of the flow (like the Actions, channels, Human Agent,): you can just put it at the correct place and near the element.
    Also before moving to the next phase/step in the flow, we must ensure that the changes are saved: so it means we might not need a button in the step to save changes for some steps (identify the ones that match best for that).
